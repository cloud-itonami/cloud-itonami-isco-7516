(ns tobaccocoord.actor
  "TobaccoCoordActor — the ISCO-08 7516 tobacco preparation and
  tobacco-products manufacturing factory-floor scheduling/logistics
  coordination actor as a `langgraph.graph/state-graph` (ADR-2607121000
  / CLAUDE.md Actors section). One graph run = one factory-floor
  ...
  an interrupted run can resume after human sign-off. Modeled closely
  on cloud-itonami-isco-7211's foundrycoord.actor for the
  ...
  The unconditional invariant: the Tobacco Preparation Coordination
  Advisor can never directly commit an operating record the
  TobaccoCoordGovernor refuses, and can never make this actor finalize
  a manufacturing-execution decision (e.g. deciding to proceed with a
  specific production run) or a regulatory-compliance-clearance
  decision (e.g. affixing a tax stamp or finalizing a
  labeling-compliance determination), or override a shop safety
  officer's judgment — every commit-record! call is gated behind
  `:decide`, and that decision class is a permanent hard block
  regardless of disposition path. This actor coordinates FACTORY-FLOOR
  SCHEDULING/LOGISTICS ONLY — it never performs tobacco-preparation or
...
(defn build-graph
  "Build a compiled TobaccoCoordActor graph. `store` implements
  `tobaccocoord.store/Store`. `advisor` implements
  `tobaccocoord.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:preparer-id (:preparer-id request)
                                    :op (:op proposal)
